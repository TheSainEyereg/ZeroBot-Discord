...
module.exports = {
	...
	async execute(message, args) {
		...
		try {
			const background = await Canvas.loadImage("./assets/images/tombstone.png");
		...
		ctx.fillText(username , (canvas.width/2-20) - (ctx.measureText(username).width / 2), 245);
		ctx.fillText(time , (canvas.width/2-20) - (ctx.measureText(time).width / 2), 290);
