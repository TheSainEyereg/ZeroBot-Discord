...
export default class Leave extends Command {
	...
	private leave(member: GuildMember) {
		...
		if (!queue)
			return critical("Guild music queue is absent", "Disconnect bot from the voice channel manually");

		if (queue.connection?.state.status != VoiceConnectionStatus.Ready)
			return warning("Bot isn't in a voice channel");

		if (!channel)
			return warning("You must be in a voice channel");
		if (channel != queue.voiceChannel)
			return warning("You must be in the same voice channel");
